...
export default function CreateAccountPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate("/otp1"); // Redirect to your OTP verification page
    } catch (err: unknown) {
      ...
      <div className="w-full lg:w-[45%] bg-white flex flex-col items-center justify-center p-6 sm:p-8 lg:p-12 overflow-y-auto">
        ...
        <div className="flex justify-center space-x-4">
          {[apple, google, Facebook].map((icon, i) => (
            <button
              key={i}
              className="w-12 h-12 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200 transition-all"
            >
