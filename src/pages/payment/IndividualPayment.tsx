...
import {
  IndividualForm,
  FamilyForm,
} from "../../components/payment/PaymentForms";
...
function IndividualPayment() {
  ...
  const { selectedCourse } = usePayment();

  const storedPlan = {
    type: sessionStorage.getItem("paymentPlan"),
  ...
  return (
    // CHANGE 1: Reduced padding for mobile (px-6) and increased for desktop (lg:px-20)
    // Added min-h-screen to ensure the gradient looks good
    <div className="text-center py-10 md:py-20 px-6 lg:px-20 min-h-screen bg-white">
      <div className="max-w-screen-lg mx-auto">
        {/* CHANGE 2: Removed fixed vh width for back button to prevent it disappearing/squashing */}
        ...
        <div className="mb-10 text-left md:text-center">
          ...
          <h1 className="text-3xl md:text-[40px] font-bold text-slate-gray">
            Make Payment
          </h1>
          <p className="text-base md:text-[18px] text-[#7F8C8D]">
            Kindly enter the correct details to make payment for your selected
            course
          </p>
        </div>

        {/* CHANGE 4: flex-col on mobile, flex-row on desktop. Removed fixed h-[276px] */}
        <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
          {/* Left Side: Summary Boxes */}
          <div className="flex flex-col w-full lg:w-[48%]">
            <div className="mb-4">
              <p className="text-sub-gray text-xs text-left mb-2">Course</p>
              <div className="border border-primary-green px-4 py-3 text-sm text-slate-gray rounded-[10px] text-left">
          ...
          <div className="w-full lg:w-[48%] flex flex-col">
            <div className="w-full mb-4">
              ...
              <input
                ...
                className={`border px-4 py-3 w-full rounded-[10px] focus:outline-primary-green ${
                  cardName
                    ? "outline-primary-green border-primary-green "
                    : "border-sub-gray"
                }`}
                value={cardName}
                onChange={(e) => {
                  ...
                  setIsFormFilled((prev) =>
                    prev.map((form, i) => (i === 0 ? value : form))
                  );
                }}
              />
            </div>
            ...
            <div className="w-full mb-4">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  let display = value;
                  if (value.length > 4) {
                    display = value.match(/.{1,4}/g)?.join(" - ") || value;
                  }
                  setCardNumnber(display);
                  setIsFormFilled((prev) =>
                    prev.map((form, i) => (i === 1 ? value : form))
                  );
                }}
              />
            </div>
            ...
            <div className="flex w-full gap-4 mb-6">
              {/* Using gap-4 provides a consistent gutter that won't push items off screen */}

              <div className="flex flex-col flex-1">
                {" "}
                {/* flex-1 ensures they stay equal size */}
                <p className="text-sub-gray text-xs text-left mb-2">
                  Expiry date
                </p>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setIsFormFilled((prev) =>
                      prev.map((form, i) => (i === 2 ? value : form))
                    );
                  }}
                />
              </div>
              ...
              <div className="flex flex-col flex-1">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setCvv(value);
                    setIsFormFilled((prev) =>
                      prev.map((form, i) => (i === 3 ? value : form))
                    );
                  }}
                />
              </div>
        ...
        <div className="w-full mt-6">
          {storedPlan.type === "Individual" ? (
            <IndividualForm canSubmit={canSubmit} />
          ) : (
...
//             <p className="text-sub-gray text-xs text-left mb-2">Plan</p>
//             <div className="border-1 border-primary-green px-4 py-[10px] text-sm text-slate-gray rounded-[10px] mb-5 text-left">{storedPlan.type}</div>
//             <p className="text-sub-gray text-xs text-left mb-2">Price</p>
//             <div className="border-1 border-primary-green px-4 py-[10px] text-sm text-slate-gray rounded-[10px] text-left mb-5">{storedPlan.price}</div>
//           </div>
...
//                 value={cardName}
//                 onChange={(e) => {
//                   const value = e.target.value.replace(/[^a-z ]/gi, "").slice(0, 100);
//                   setCardName(value);
...
//               <input type="text" name="myCustomfield" autoComplete="off" className={`border-1 px-4 py-[10px] mb-6 w-full rounded-[10px] focus:outline-primary-green ${cardNumber ? 'outline-primary-green border-primary-green ' : 'border-sub-gray'}`}
//                 value={cardNumber}
//                 onChange={(e) => {
//                   const value = e.target.value.replace(/\D/g, "").slice(0, 16);
//                   let display = value
...
//           {(storedPlan.type == 'Individual') ? <IndividualForm canSubmit={canSubmit} /> :
//             <FamilyForm canSubmit={canSubmit} />}
//         </div>
//       </div>
//     </div>
